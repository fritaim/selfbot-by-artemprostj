...
class Tools(commands.Cog):
	...
	async def status(self, ctx, cat='ы', *, name='MainWars'):
		...
		if cat in game:
			...
			await self.bot.change_presence(status=sstatus, activity=discord.Streaming(name=name, url="https://www.twitch.tv/404%27"))
		elif cat in reset:
			await self.bot.change_presence(status=sstatus, activity=None)
		elif cat in discordd:
			await self.bot.change_presence(status=sstatus, activity=discord.Activity(type=discord.ActivityType.playing, application_id=1029430390357774388, name='Discord', assets={'large_image': '1029438970666426408', 'large_text': 'Selfbot by artemprostj\nbit.ly/3CXNRpC'}))
		elif cat in selfbot:
			await self.bot.change_presence(status=sstatus, activity=discord.Activity(type=discord.ActivityType.playing, application_id=1032671485120229397, name='Selfbot by artemprostj', details='bit.ly/3CXNRpC', assets={'large_image': '1032672678106116216', 'large_text': 'Selfbot by artemprostj\nbit.ly/3CXNRpC'}))
		elif cat in mainwars:
			await self.bot.change_presence(status=sstatus, activity=discord.Streaming(type=discord.ActivityType.streaming, application_id=1029101222445199530, name='Привет)', details='Привет)', url="https://www.twitch.tv/404%27", assets={'large_text': 'Привет привет)))'}))
		else:
		...
		for i in range(amount):
			while True:
				...
				if response.status_code==200 or response.status_code==201:
					id=response.json()['id']
					requests.post(f"https://discord.com/api/v9/channels/{id}/messages", headers={'authorization': self.bot.http.token}, json={"content": "||@everyone||**__Selfbot by artemprostj\nhttps://github.com/Its-artemprostj/Discord-Selfbot __**"})
					break
				elif response.status_code==429:
					seconds=response.json()['retry_after']
					if 100>seconds:
					...
					await ctx.send(content=f"**__Selfbot by artemprostj__\n\nПроизошла ошибка :x:\n```Код ошибки: {response.status_code}\n{response.text}```**")
					return
